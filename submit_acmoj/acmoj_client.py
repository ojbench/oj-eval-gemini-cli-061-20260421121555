...
def main():
    ...
    submit_parser.add_argument("--code-file", type=str, help="Path to the source code file")
    submit_parser.add_argument("--git-url", type=str, help="Git repository URL (if language is git)")
    ...
    if args.command == "submit":
        if args.language == "git":
            if not args.git_url:
                print("Error: --git-url is required when language is git")
                exit(1)
            result = client.submit_git(args.problem_id, args.git_url)
        else:
            if not args.code_file:
                print("Error: --code-file is required when language is not git")
                exit(1)
            try:
